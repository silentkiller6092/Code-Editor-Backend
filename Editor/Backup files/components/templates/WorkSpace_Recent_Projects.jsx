...
function WorkSpace_Recent_Projects() {
    ...
    >
      {[0, 0, 0, 0, 0, 0, 0, 0, 0].map((value, idx) => {
        return (
          <div
            key={idx}
            className="alert-item flex cursor-pointer items-center py-2 px-4 mb-4 rounded-xl text-sm border border-purple-500 bg-transparent transition-all duration-300 hover:bg-slate-700 text-purple-500 hover:text-white"
            role="alert"
            style={{ marginRight: "10px" }} // Add margin to create distance from scrollbar
          >
            <IconWorldWww className="mt-1 mr-1" />
            <p className="font-semibold mr-1 text-purple-500 hover:text-white">
              <span>Success</span>{" "}
...
export default WorkSpace_Recent_Projects;
